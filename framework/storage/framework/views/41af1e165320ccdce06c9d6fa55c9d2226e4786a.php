...
$redirect = url('/').'/admin/products/'.$rel_field.'/'.$rel_id.'?'.urlencode($_SERVER["QUERY_STRING"]);
?>
<!-- page content -->
<div class="right_col" role="main">
  <div class="row x_panel">
    ...
    <div class="pull-left">
      <a href="<?php echo url('/'); ?>/admin/products/add/<?php echo $rel_field; ?>/<?php echo $rel_id; ?>" class="btn btn-success">Add Products</a>
    </div>
    <br>
    <hr>
    ...
    <form method="GET" action="" class="form-inline ibox-content">
        <div class="form-group">
          <select name="searchBy" class="form-control">
           <option value="products.name" <?php echo $searchBy=="products.name"?'selected="selected"':""; ?>>Name</option>
          </select>
        </div>
        <div class="form-group">
    ...
    <div class="table-responsive">
      ...
      <th><input onclick="toggle(this,'cbgroup1')" id="foo[]" name="foo[]" type="checkbox" value="" /></th>
       <th> Sr No. </th>
          <?php $sortSym=isset($_GET["order"]) && $_GET["order"]=="asc" ? "up" : "down"; ?>
            <?php $symbol = isset($_GET["sortBy"]) && $_GET["sortBy"]=="products.name"?"<i class='fa fa-sort-$sortSym' aria-hidden='true'></i>": "<i class='fa fa-sort' aria-hidden='true'></i>"; ?>
            <th>
            <a href="<?php echo $links["products.name_link"]; ?>" class="link_css"> Name <?php echo $symbol ?></a>
            </th>
      ...
      <?php $count=1;
          ...
          foreach($data as $value){
           ?>
          <tr id="hide<?php $value->pro_id; ?>" >
            
            <th>
            <input name='input' id='del' onclick="callme('show')"  type='checkbox' class='del' value='<?php echo $value->pro_id; ?>'/>
            </th>
            <th>
           ...
           <th>
           <a href="<?php echo url("/"); ?>/admin/products/view/<?php echo $value->pro_id?>" title="View">
            <span class="btn btn-info " ><i class="fa fa-eye"></i></span>
           </a>
           <a href="<?php echo url("/"); ?>/admin/products/edit/<?php echo $value->pro_id; ?>/<?php echo $rel_field; ?>/<?php echo $rel_id; ?>" title="Edit">
            <span class="btn btn-info " ><i class="fa fa-edit"></i></span>
           </a>
           <a  title="Delete" data-toggle="modal" data-target="#commonDelete" onclick="$('#set_commondel_id').val('<?php echo $value->pro_id; ?>');">
...
<div class="modal fade" id="commonDelete" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
 <form action="<?php echo url('/'); ?>/admin/products/delete" method="post">
  <div class="modal-dialog">
   ...
   Do you really want to delete?
    ...
    <input type="hidden" id="set_commondel_id" name="pro_id">
...
<script type="text/javascript">
   ...
   {var confrm = confirm("Are you sure you want to delete?");
     if(confrm)
     {
       ...
       $.ajax({
         type:"POST",
         url:'<?php echo url("/")."/admin/products/deleteAll"; ?>',
         data:{
          allIds:ids,
