...
$redirect = url('/').'/admin/items/'.$rel_field.'/'.$rel_id.'?';
?>
<!-- page content -->
    ...
    <form method="GET" action="" class="form-inline ibox-content">
        <div class="form-group">
          <select name="searchBy" class="form-control">
           <option value="items.description" <?php echo $searchBy=="items.description"?'selected="selected"':""; ?>>Description</option><option value="items.created_at" <?php echo $searchBy=="items.created_at"?'selected="selected"':""; ?>>Created_at</option><option value="items.updated_at" <?php echo $searchBy=="items.updated_at"?'selected="selected"':""; ?>>Updated_at</option>
          </select>
        </div>
        <div class="form-group">
      ...
      <th><input onclick="toggle(this,'cbgroup1')" id="foo[]" name="foo[]" type="checkbox" value="" /></th>
       <th> Sr No. </th>
          <?php $sortSym=isset($_GET["order"]) && $_GET["order"]=="asc" ? "up" : "down"; ?>
            <?php $symbol = isset($_GET["sortBy"]) && $_GET["sortBy"]=="items.description"?"<i class='fa fa-sort-$sortSym' aria-hidden='true'></i>": "<i class='fa fa-sort' aria-hidden='true'></i>"; ?>
            <th>
            <a href="<?php echo $links["items.description_link"]; ?>" class="link_css"> Description <?php echo $symbol ?></a>
            </th>
            
            <?php $symbol = isset($_GET["sortBy"]) && $_GET["sortBy"]=="items.created_at"?"<i class='fa fa-sort-$sortSym' aria-hidden='true'></i>": "<i class='fa fa-sort' aria-hidden='true'></i>"; ?>
            <th>
            <a href="<?php echo $links["items.created_at_link"]; ?>" class="link_css"> Created_at <?php echo $symbol ?></a>
            </th>
            
            <?php $symbol = isset($_GET["sortBy"]) && $_GET["sortBy"]=="items.updated_at"?"<i class='fa fa-sort-$sortSym' aria-hidden='true'></i>": "<i class='fa fa-sort' aria-hidden='true'></i>"; ?>
            <th>
            <a href="<?php echo $links["items.updated_at_link"]; ?>" class="link_css"> Updated_at <?php echo $symbol ?></a>
            </th>
